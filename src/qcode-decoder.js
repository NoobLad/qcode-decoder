(function (root, factory) {
  ...
  /**
   * Sets the deviceId for the camera to use.
   *
   * The deviceId can be found using the
   * getVideoSources function
   *
   * @param {String} deviceId     The id of the
   * video source you want to use (or false to use
   * the current default)
   */
  QCodeDecoder.prototype.setDeviceId = function (deviceId) {
    if (deviceId)
      ...
      return navigator.mediaDevices.enumerateDevices()
        .then(function (devices) {
          return devices.filter(function (device) {
              return device.kind.indexOf('video') >= 0
            })
        })
    } else {
